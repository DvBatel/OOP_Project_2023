...
public class LevelTwo implements LevelInformation, Sprite {
        ...
        for (int i = 0; i < Const.TEN; i++) {
            if (i > Const.FOUR) {
                this.initialBallVelocities.add(Velocity.fromAngleAndSpeed(
                        Angle.degrees()[Const.NINE - i],
                        BALL_SPEED + Const.TWO));
            } else {
                this.initialBallVelocities.add(Velocity.fromAngleAndSpeed(
    ...
    public void drawOn(DrawSurface d) {
        ...
        int centerX = Const.TWO_HUNDRED;
        int centerY = Const.HUNDRED_FIFTY;
        int radius = Const.THREE_HUNDRED;
        int numLines = Const.SEVENTY_TWO;
